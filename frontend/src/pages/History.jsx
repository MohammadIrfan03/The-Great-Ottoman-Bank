  ...
  return (
    <>
      ...
      <div className="content">
        <div className="card">
          {loading && <p>Loading…</p>}
          {!loading && transactions.length === 0 && <p style={{ fontSize: 13.5, opacity: 0.6 }}>No transactions found.</p>}
          {!loading && transactions.length > 0 && (
            <table>
              <thead>
                <tr><th>Reference</th><th>Type</th><th>From</th><th>To</th><th>Status</th><th>Date</th><th style={{ textAlign: 'right' }}>Amount</th></tr>
              </thead>
              <tbody>
                {transactions.map((t) => {
                  ...
                  return (
                    <tr key={t.id}>
                      ...
                      <td>{t.type}</td>
                      <td className="financial-number">{t.fromAccount || '—'}</td>
                      <td className="financial-number">{t.toAccount || '—'}</td>
                      <td><span className={`badge ${t.status === 'SUCCESS' ? 'success' : 'failed'}`}>{t.status}</span></td>
                      <td className="financial-number">{new Date(t.createdAt).toLocaleString()}</td>
